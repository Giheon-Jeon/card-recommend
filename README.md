...
5. **카드 데이터 구조 유효성 검증**: 자체 검증 스크립트를 통해 수동으로 작성한 카드 데이터 파일 포맷이 올바른지 자동으로 진단합니다.
...
npm run validate:cards  # 수동 작성한 data/cards/*.json 스키마 검증
```
...
## 📂 카드 데이터 직접 입력하기
`data/cards/` 폴더 내에 JSON 형식으로 전월실적 구간까지 포함한 카드 데이터를 작성해 배치할 수 있습니다. (현재 UI는 카탈로그 기반으로 동작하므로, 이 폴더의 파일은 `npm run validate:cards`로 형식만 검증되며 화면에는 자동으로 반영되지 않습니다.)
